...
export function ProgressModal({ open, steps, currentStep, progress, error }: ProgressModalProps) {
  if (!open) return null
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" style={{ animation: 'fadeIn 0.2s ease' }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" style={{ animation: 'slideUp 0.3s ease' }}>
        ...
        <div className="bg-gradient-to-r from-[var(--teuco-green)] to-[var(--teuco-green-light)] p-6 text-center">
          ...
          <h2 className="text-xl font-black text-white uppercase tracking-tight">Atualizando Site</h2>
          <p className="text-white/80 text-xs font-bold uppercase tracking-wider mt-1">Aguarde um momento...</p>
        </div>
